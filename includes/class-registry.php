...
class Registry {
	...
	private function register_single_post_type( string $slug, array $config ): void {
		...
		$labels = [
			...
			'add_new'                  => __( '新建', 'wem-content-types' ),
			'add_new_item'             => sprintf( __( '新建%s', 'wem-content-types' ), $singular ),
			'edit_item'                => sprintf( __( '编辑%s', 'wem-content-types' ), $singular ),
			'new_item'                 => sprintf( __( '新建%s', 'wem-content-types' ), $singular ),
			'view_item'                => sprintf( __( '查看%s', 'wem-content-types' ), $singular ),
			'view_items'               => sprintf( __( '查看%s', 'wem-content-types' ), $plural ),
			'search_items'             => sprintf( __( '搜索%s', 'wem-content-types' ), $plural ),
			'not_found'                => sprintf( __( '未找到%s。', 'wem-content-types' ), strtolower( $plural ) ),
			'not_found_in_trash'       => sprintf( __( '回收站中未找到%s。', 'wem-content-types' ), strtolower( $plural ) ),
			'all_items'                => sprintf( __( '所有%s', 'wem-content-types' ), $plural ),
			'archives'                 => sprintf( __( '%s归档', 'wem-content-types' ), $singular ),
			'attributes'               => sprintf( __( '%s属性', 'wem-content-types' ), $singular ),
			'insert_into_item'         => sprintf( __( '插入到%s', 'wem-content-types' ), strtolower( $singular ) ),
			'uploaded_to_this_item'    => sprintf( __( '上传到此%s', 'wem-content-types' ), strtolower( $singular ) ),
			'featured_image'           => sprintf( __( '%s特色图片', 'wem-content-types' ), $singular ),
			'set_featured_image'       => sprintf( __( '设置%s特色图片', 'wem-content-types' ), strtolower( $singular ) ),
			'remove_featured_image'    => sprintf( __( '移除%s特色图片', 'wem-content-types' ), strtolower( $singular ) ),
			'use_featured_image'       => sprintf( __( '用作%s特色图片', 'wem-content-types' ), strtolower( $singular ) ),
			'filter_items_list'        => sprintf( __( '筛选%s列表', 'wem-content-types' ), strtolower( $plural ) ),
			'items_list_navigation'    => sprintf( __( '%s列表导航', 'wem-content-types' ), $plural ),
			'items_list'               => sprintf( __( '%s列表', 'wem-content-types' ), $plural ),
		];
	...
	private function register_single_taxonomy( string $slug, array $config ): void {
		...
		$labels = [
			...
			'menu_name'                  => $plural,
			'search_items'               => sprintf( __( '搜索%s', 'wem-content-types' ), $plural ),
			'popular_items'              => sprintf( __( '热门%s', 'wem-content-types' ), $plural ),
			'all_items'                  => sprintf( __( '所有%s', 'wem-content-types' ), $plural ),
			'parent_item'                => sprintf( __( '父级%s', 'wem-content-types' ), $singular ),
			'parent_item_colon'          => sprintf( __( '父级%s：', 'wem-content-types' ), $singular ),
			'edit_item'                  => sprintf( __( '编辑%s', 'wem-content-types' ), $singular ),
			'view_item'                  => sprintf( __( '查看%s', 'wem-content-types' ), $singular ),
			'update_item'                => sprintf( __( '更新%s', 'wem-content-types' ), $singular ),
			'add_new_item'               => sprintf( __( '新建%s', 'wem-content-types' ), $singular ),
			'new_item_name'              => sprintf( __( '新%s名称', 'wem-content-types' ), $singular ),
			'separate_items_with_commas' => sprintf( __( '使用逗号分隔%s', 'wem-content-types' ), strtolower( $plural ) ),
			'add_or_remove_items'        => sprintf( __( '添加或移除%s', 'wem-content-types' ), strtolower( $plural ) ),
			'choose_from_most_used'      => sprintf( __( '从最常用的%s中选择', 'wem-content-types' ), strtolower( $plural ) ),
			'not_found'                  => sprintf( __( '未找到%s。', 'wem-content-types' ), strtolower( $plural ) ),
			'no_terms'                   => sprintf( __( '无%s', 'wem-content-types' ), strtolower( $plural ) ),
			'filter_by_item'             => sprintf( __( '按%s筛选', 'wem-content-types' ), $singular ),
			'items_list_navigation'      => sprintf( __( '%s列表导航', 'wem-content-types' ), $plural ),
			'items_list'                 => sprintf( __( '%s列表', 'wem-content-types' ), $plural ),
			'back_to_items'              => sprintf( __( '← 返回%s', 'wem-content-types' ), $plural ),
		];
